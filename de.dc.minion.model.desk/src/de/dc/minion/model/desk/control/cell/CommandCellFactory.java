package de.dc.minion.model.desk.control.cell;
...
public class CommandCellFactory extends ListCell<EmfCommand> {

	private Logger log = Logger.getLogger(CommandCellFactory.class.getSimpleName());
	
	public static final String ICON_PATH = "/de/dc/minion/model/desk/control/icons/cell/";
	...
	public CommandCellFactory(ComposedAdapterFactory adapterFactory) {
	...
	protected void updateItem(EmfCommand item, boolean empty) {
		...
		if (empty || item == null) {
			...
			if (mLLoader == null) {
				mLLoader = new FXMLLoader(getClass().getResource("/de/dc/minion/model/desk/control/cell/CommandCellFactory.fxml"));
			...
			if (item instanceof EmfCommand) {
				...
				setGraphic(root);
			}else if( item instanceof CommandHistoryTouch){
	...
	private String getCommandName(EmfCommand emfCommand) {
		...
		if (command instanceof SetCommand) {
			...
			imageView.setImage(new Image(getClass().getResourceAsStream(ICON_PATH+"icons8-file-32-edit.png")));
		} else if (command instanceof ChangeCommand) {
			name = command.getDescription();
			imageView.setImage(new Image(getClass().getResourceAsStream(ICON_PATH+"icons8-file-32-change.png")));
			...
			name = "Add new " + owner;
			imageView.setImage(new Image(getClass().getResourceAsStream(ICON_PATH+"icons8-file-32-add.png")));
			...
			imageView.setImage(new Image(getClass().getResourceAsStream(ICON_PATH+"icons8-file-32-delete.png")));
			...
			name = "DND for " + owner;
			imageView.setImage(new Image(getClass().getResourceAsStream(ICON_PATH+"icons8-file-32-dnd.png")));
		}else {
			name = command.getDescription();
			imageView.setImage(new Image(getClass().getResourceAsStream(ICON_PATH+"icons8-file-16.png")));
		}
		return name;
	}
