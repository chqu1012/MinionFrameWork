package de.dc.minion.model.editor
...
class CreateIdeFileDialog extends TitleAreaDialog {
	...
	override protected Control createDialogArea(Composite parent) {
		setMessage("Create generation.minion files with following properties.")
		setTitle("Generation Minion File Creator")
				...
				for (URI uri : dialog.getURIs()) {
					...
					if (eObject instanceof XPackage) {
						...
						if (adapter instanceof XPackageMapping) {
							var m = adapter as org.eclipse.emf.ecore.xcore.mappings.XPackageMapping
							var basePackage = m.getGenPackage.basePackage
