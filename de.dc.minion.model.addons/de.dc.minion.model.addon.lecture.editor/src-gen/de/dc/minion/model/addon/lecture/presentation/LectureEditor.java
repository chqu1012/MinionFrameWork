...
import de.dc.minion.model.addon.lecture.provider.LectureItemProviderAdapterFactory;
...
/**
 * This is an example of a Lecture model editor.
 * <!-- begin-user-doc -->
 * <!-- end-user-doc -->
 * @generated
 */
public class LectureEditor extends MultiPageEditorPart
	 ...
	 * @generated
	 */
	public static final List<String> FILE_EXTENSION_FILTERS = prefixExtensions(LectureModelWizard.FILE_EXTENSIONS,
			"*.");
		...
		public void partActivated(IWorkbenchPart p) {
			if (p instanceof ContentOutline) {
				if (((ContentOutline) p).getCurrentPage() == contentOutlinePage) {
					getActionBarContributor().setActiveEditor(LectureEditor.this);
	...
	protected void handleActivate() {
		...
		if (!removedResources.isEmpty()) {
			if (handleDirtyConflict()) {
				getSite().getPage().closeEditor(LectureEditor.this, false);
			} else {
	...
	protected void updateProblemIndication() {
		if (updateProblemIndication) {
			BasicDiagnostic diagnostic = new BasicDiagnostic(Diagnostic.OK, "de.dc.minion.model.addon.lecture.editor",
	...
	protected void initializeEditingDomain() {
		...
		adapterFactory.addAdapterFactory(new LectureItemProviderAdapterFactory());
	...
	public Diagnostic analyzeResourceProblems(Resource resource, Exception exception) {
		...
		if (hasErrors || !resource.getWarnings().isEmpty()) {
			BasicDiagnostic basicDiagnostic = new BasicDiagnostic(hasErrors ? Diagnostic.ERROR : Diagnostic.WARNING,
					"de.dc.minion.model.addon.lecture.editor", 0,
					getString("_UI_CreateModelError_message", resource.getURI()),
					new Object[] { exception == null ? (Object) resource : exception });
			basicDiagnostic.merge(EcoreUtil.computeDiagnostic(resource, true));
			return basicDiagnostic;
		} else if (exception != null) {
			return new BasicDiagnostic(Diagnostic.ERROR, "de.dc.minion.model.addon.lecture.editor", 0,
		...
		if (!getEditingDomain().getResourceSet().getResources().isEmpty()) {
			...
			selectionViewer.setLabelProvider(new DecoratingColumLabelProvider(
					new AdapterFactoryLabelProvider(adapterFactory), new DiagnosticDecorator(editingDomain,
							selectionViewer, LectureEditorPlugin.getPlugin().getDialogSettings())));
	...
	public <T> T getAdapter(Class<T> key) {
		if (key.equals(IContentOutlinePage.class)) {
			...
			return FindAndReplaceTarget.getAdapter(key, this, LectureEditorPlugin.getPlugin());
		} else {
			...
			class MyContentOutlinePage extends ContentOutlinePage {
				...
				public void createControl(Composite parent) {
					...
					contentOutlineViewer.setLabelProvider(new DecoratingColumLabelProvider(
							new AdapterFactoryLabelProvider(adapterFactory), new DiagnosticDecorator(editingDomain,
									contentOutlineViewer, LectureEditorPlugin.getPlugin().getDialogSettings())));
	...
	public IPropertySheetPage getPropertySheetPage() {
		PropertySheetPage propertySheetPage = new ExtendedPropertySheetPage(editingDomain,
				ExtendedPropertySheetPage.Decoration.LIVE, LectureEditorPlugin.getPlugin().getDialogSettings(), 1,
				true) {
			@Override
			public void setSelectionToViewer(List<?> selection) {
				LectureEditor.this.setSelectionToViewer(selection);
				LectureEditor.this.setFocus();
			}
	...
	public void doSaveAs() {
		...
		String[] files = LectureEditorAdvisor.openFilePathDialog(getSite().getShell(), SWT.SAVE, filters);
		if (files.length > 0) {
	...
	private static String getString(String key) {
		return LectureEditorPlugin.INSTANCE.getString(key);
	}
	...
	private static String getString(String key, Object s1) {
		return LectureEditorPlugin.INSTANCE.getString(key, new Object[] { s1 });
	}
