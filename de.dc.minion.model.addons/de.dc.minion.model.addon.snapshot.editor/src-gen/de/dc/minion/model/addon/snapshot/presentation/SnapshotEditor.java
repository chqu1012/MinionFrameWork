...
import de.dc.minion.model.addon.snapshot.provider.SnapshotItemProviderAdapterFactory;
 ...
 * @generated
 */
public class SnapshotEditor extends MultiPageEditorPart
	 ...
	 * @generated
	 */
	public static final List<String> FILE_EXTENSION_FILTERS = prefixExtensions(SnapshotModelWizard.FILE_EXTENSIONS,
			"*.");
	...
	protected void updateProblemIndication() {
		if (updateProblemIndication) {
			BasicDiagnostic diagnostic = new BasicDiagnostic(Diagnostic.OK, "de.dc.minion.model.addon.snapshot.editor",
	...
	public Diagnostic analyzeResourceProblems(Resource resource, Exception exception) {
		...
		if (hasErrors || !resource.getWarnings().isEmpty()) {
			BasicDiagnostic basicDiagnostic = new BasicDiagnostic(hasErrors ? Diagnostic.ERROR : Diagnostic.WARNING,
					"de.dc.minion.model.addon.snapshot.editor", 0,
					getString("_UI_CreateModelError_message", resource.getURI()),
					new Object[] { exception == null ? (Object) resource : exception });
			basicDiagnostic.merge(EcoreUtil.computeDiagnostic(resource, true));
			return basicDiagnostic;
		} else if (exception != null) {
			return new BasicDiagnostic(Diagnostic.ERROR, "de.dc.minion.model.addon.snapshot.editor", 0,
		...
		if (!getEditingDomain().getResourceSet().getResources().isEmpty()) {
			...
			selectionViewer.setLabelProvider(new DecoratingColumLabelProvider(
					new AdapterFactoryLabelProvider(adapterFactory), new DiagnosticDecorator(editingDomain,
							selectionViewer, SnapshotEditorPlugin.getPlugin().getDialogSettings())));
	...
	public <T> T getAdapter(Class<T> key) {
		if (key.equals(IContentOutlinePage.class)) {
			...
			return FindAndReplaceTarget.getAdapter(key, this, SnapshotEditorPlugin.getPlugin());
		} else {
			...
			class MyContentOutlinePage extends ContentOutlinePage {
				...
				public void createControl(Composite parent) {
					...
					contentOutlineViewer.setLabelProvider(new DecoratingColumLabelProvider(
							new AdapterFactoryLabelProvider(adapterFactory), new DiagnosticDecorator(editingDomain,
									contentOutlineViewer, SnapshotEditorPlugin.getPlugin().getDialogSettings())));
	...
	public IPropertySheetPage getPropertySheetPage() {
		PropertySheetPage propertySheetPage = new ExtendedPropertySheetPage(editingDomain,
				ExtendedPropertySheetPage.Decoration.LIVE, SnapshotEditorPlugin.getPlugin().getDialogSettings(), 1,
				true) {
			@Override
	...
	public void doSaveAs() {
		...
		String[] files = SnapshotEditorAdvisor.openFilePathDialog(getSite().getShell(), SWT.SAVE, filters);
		if (files.length > 0) {
